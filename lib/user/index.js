...
module.exports = function user(user, options) {
	...
	return {
		...
		isFollowing:function(target, cb){
			isFollowing(target,cb);
		},
		isFollowedBy:function(target,cb){
			isFollowedBy(target,cb);
		}
	}
	...
	function isFollowing(target,cb){
		client.sismember(keys.followingKey(user),target,cb)
	}

	function isFollowedBy(target,cb){
		client.sismember(keys.followerKey(user), target, cb)
	}
